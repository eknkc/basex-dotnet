...
namespace BaseX.Tests
{
  ...
  {
    static (string, string, string)[] testRules = new (string, string, string)[] {
        ("01", "000f", "01111"),
        ("01", "00ff", "011111111"),
    ...
    public void TestContructor()
    {
      ...
      Assert.NotNull(new Encoding("01"));
    }

    [Fact]
    public void TestEncode()
    {
      foreach (var (alphabet, data, encoded) in testRules)
      {
        var enc = new Encoding(alphabet);
        Assert.Equal(encoded, enc.Encode(ToByteArray(data)));
      }
    }
    ...
    public void TestDecode()
    {
      ...
      {
        var enc = new Encoding(alphabet);
        Assert.Equal(ToByteArray(data), enc.Decode(encoded));
      }
    }
    ...
    {
      var enc = new Encoding("123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz");
      ...
      Assert.Throws<ArgumentException>(() =>
      {
        enc.Decode("c2F0b3NoaQo=");

      });

      Assert.Throws<ArgumentException>(() =>
      {
        enc.Decode(" 1111111111");
      });
